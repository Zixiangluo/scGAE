...
def load_data(N, norm="scanpy"):
    if norm == "raw":
        count = np.array(
            pd.read_csv("/Users/zixiangluo/Desktop/DR/Data/data" + str(N) + "/data" + str(N) + ".csv",
                        index_col=0, sep="\t"))
    elif norm == "scanpy":
        count = np.array(
            pd.read_csv("/Users/zixiangluo/Desktop/DR/Data/data" + str(N) + "/data" + str(N) + ".csv",
    ...
    idents = np.array(
        pd.read_csv("/Users/zixiangluo/Desktop/DR/Data/data" + str(N) + "/group" + str(N) + ".csv", index_col=0,
                    sep="\t")).reshape(-1, )
    return count, idents


def get_adj(count, k=30, pca=80, mode="connectivity"):
    if pca:
