...
from tensorflow.keras.losses import MSE, binary_crossentropy, KLD
from tensorflow.keras.layers import InputLayer, Dense, Dropout, Input, Lambda
...
class SCGAE(tf.keras.Model):
    ...
    def __init__(self, X, adj, adj_n, hidden_dim=64, latent_dim=12, dec_dim=None, adj_dim=32,
                 decA="DBL", layer_enc = "TAG"):
    ...
    def pre_train(self, epochs=100, info_step=10, lr=2e-3, W_a=0.1, W_x=1,
                  W_d=0, min_dist=0.5, max_dist=20, cut_loss=False):
        ...
        for epoch in range(1, epochs + 1):
            with tf.GradientTape(persistent=True) as tape:
                ...
                if W_d:
                    Dist_loss = tf.reduce_mean(dist_loss(z, min_dist, cut=cut_loss, max_dist = max_dist))
    ...
    def alt_train(self, epochs=100, lr=2e-3, W_a=0.2, W_x=1, W_c=1, info_step=10, n_update=10, centers=None):
